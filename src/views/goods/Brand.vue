<template>
 ...
 <el-row :gutter="20">
   ...
   <el-col :span="4">
     <el-button type="primary" @click="showAddDialog">添加品牌</el-button>
   </el-col>
 </el-row>
        <!-- 标签列表区域 -->
        <el-table :data="dataList" border stripe>
          <el-table-column type="index"></el-table-column>
          <el-table-column label="商品品牌名" prop="name"></el-table-column>
  ...
  export default {
      ...
      return {
         ...
         formRules: {
           name: [{required: true, message: '品牌名称', trigger: 'blur' },
                  {min: 3,max: 50,message: '品牌名称的长度在3~50个字符之间',trigger: 'blur'}],
         },
    ...
    methods: {
     getAddForm(){
       ...
       async getDataList() {
         const { data: res } = await this.$http.get('apiBrand', {params: this.queryInfo})
         if (res.code !== 200) {
    ...
    add() {
       ...
       if (!valid) return
       const { data: res } = await this.$http.post('apiBrand', this.addForm)
       ...
       async showEditDialog(id) {
         const { data: res } = await this.$http.get('apiBrand/' + id)
      ...
      edit() {
          this.$refs.editFormRef.validate(async valid => {
            if (!valid) return
            const { data: res } = await this.$http.put('apiBrand/' + this.editForm.id,this.editForm)
            if (res.code !== 200) {return this.$message.error(res.msg)}
       ...
       async removeById(id) {
         ...
         const { data: res } = await this.$http.delete('apiBrand/' + id)
         if (res.code !== 200) {
           return this.$message.error(res.msg)
         }
