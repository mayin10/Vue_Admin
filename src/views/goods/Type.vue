  ...
  <div>
    ...
    <el-card>
        <el-row :gutter="20">
          ...
          <el-col :span="4">
            <el-button type="primary" @click="showAddDialog">添加规格模型</el-button>
          </el-col>
        </el-row>
        <!-- 标签列表区域 -->
    ...
    <!-- 添加对话框 -->
    <el-dialog title="添加规格模型" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
      <!-- 内容主体区域 -->
      <el-form :model="addForm" :rules="formRules" ref="addFormRef" label-width="150px">
        <el-form-item label="规格模型名" prop="name">
          <el-input v-model="addForm.name"></el-input>
        </el-form-item>
        <el-form-item label="规格模型简介">
            <el-input type="textarea" v-model="addForm.desc" />
        </el-form-item>

        <el-form-item label="图片 (530*180)">
           <div>只可以上传一张图片</div>
    ...
    <el-dialog title="修改规格模型" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
...
<script>
   ...
   data() {
     return {
        ...
        addForm: {},
        formRules: {
          name: [{required: true, message: '规格模型名称', trigger: 'blur' },
                 {min: 3,max: 50,message: '规格模型名称的长度在3~50个字符之间',trigger: 'blur'}],
        },
   ...
   methods: {
     getAddForm(){
       ...
       async getDataList() {
         const { data: res } = await this.$http.get('apiType', {params: this.queryInfo})
         if (res.code !== 200) {
       ...
       showAddDialog(){
        ...
        async hotChange(cate) {
          const {data: res} = await this.$http.post('apiTypeHot', {id: cate.id, is_hot: cate.is_hot?1:0})
          if (res.code !== 200) {
       ...
       add() {
          ...
          if (!valid) return
          const { data: res } = await this.$http.post('apiType', this.addForm)
         ...
         async showEditDialog(id) {
           const { data: res } = await this.$http.get('apiType/' + id)
          ...
          edit() {
            this.$refs.editFormRef.validate(async valid => {
              if (!valid) return
              const { data: res } = await this.$http.put('apiType/' + this.editForm.id,this.editForm)
              if (res.code !== 200) {return this.$message.error(res.msg)}
          ...
          async removeById(id) {
            ...
            const { data: res } = await this.$http.delete('apiType/' + id)
            if (res.code !== 200) {
              return this.$message.error(res.msg)
            }
